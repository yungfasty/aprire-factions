...
import com.github.akafasty.aprire.inventories.KickInventory;
...
public class KickCommand extends SubCommand {
    ...
    public void execute(Player player, String[] arguments) {
        ...
        if (arguments.length != 2) { player.sendMessage("§cUse \"/f expulsar <jogador>\"."); return; }
        ...
        if (targetUser == null) { player.sendMessage("§cEste jogador não faz parte da sua facção!"); return; }

        if (targetUser.getRole().getValue() >= user.getRole().getValue()) { player.sendMessage("§cVocê não pode expulsar este jogador."); return; }

        new KickInventory(player, faction.getTag(), targetUser.getName());
